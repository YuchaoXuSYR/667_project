...
class Game():
    def __init__(self):
        ...
        if type(self.size) != int or self.size == 1:
            ...
            print("invalid size, cheesboard size has been set in default")
        if self.size < 5:
            print("warning! there will be no winner in this game for the maximum in a line is less than 5.")
        self.board = np.zeros([self.size, self.size, 2])
        self.ai = tree_new.TreeAI(self.size)
    ...
    def player2(self):
        ...
        while (validPosition == False):
            ...
            print("AI's Turn")
            if self.AIflag == '1':
                score, row, col = self.ai.search(self.board, 2)
            else:
                row, col = self.ai.uniform(self.board)
            print("AI choose to put in: {x}, {y}".format(x=row, y=col))
            if row >= self.size or col >= self.size:
                print("row or column out of index!")
                continue
